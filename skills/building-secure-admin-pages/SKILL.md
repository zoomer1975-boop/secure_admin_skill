...
- During invitation setup, show the invitee three labeled QR codes: the official iOS App Store installation QR, the official Android Google Play installation QR, and one account-specific `otpauth://totp/` QR shared by both platforms. Include direct, clickable official store links without shorteners, redirects, or tracking parameters: iOS `https://apps.apple.com/us/app/google-authenticator/id388497605` and Android `https://play.google.com/store/apps/details?id=com.google.android.apps.authenticator2`; do not add device detection.
