...
test('installs the complete skill for Codex', async (t) => {
  ...
  for (const requirement of [
    ...
    /Normalize the password, login ID, and name with Unicode NFKC, case-folding, and whitespace removal.*full normalized ID or name, any contiguous three-character substring/,
    /full value when either is one or two characters long/,
    /ascending or descending digit sequences of three or more digits/,
    /leave the account, invitation, and stored hash unchanged/,
    /The creator must never know the final password/,
    ...
    /Generate the TOTP QR locally or on the application server/,
    /Never send the TOTP secret or provisioning URI to an external QR-generation service, cache, log, analytics event, or client-side storage/,
